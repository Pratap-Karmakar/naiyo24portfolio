...
export function GetInTouch() {
  ...
  return (
    <div
      ref={ref}
      className="h-[40rem] md:pt-32 lg:pt-36 pt-48 pb-10 px-4 relative overflow-hidden flex items-center justify-center"
    >
      <WavyBackground className="max-w-4xl mx-auto pb-40">
        ...
        <motion.form
          ...
          initial="hidden"
          animate={inView ? "show" : "hidden"} // Control animation based on visibility
          className="flex flex-col gap-6 w-full pt-10 mx-auto"
        >
          <div className="flex flex-col md:flex-row gap-6">
            <motion.input
              type="text"
              placeholder="Name"
              className="input w-full md:w-1/2 p-4 border border-purple-400 bg-black rounded-md focus:ring-2 focus:ring-purple-400"
          ...
          <motion.input
            type="text"
            placeholder="Subject"
            className="input w-full p-4 border bg-black border-purple-400 rounded-md focus:ring-2 focus:ring-purple-400"
            variants={fadeIn("up", 0.6)} // Shuffle effect
            initial="hidden"
            animate={inView ? "show" : "hidden"}
          />
          ...
          <motion.textarea
            placeholder="Message"
            className="textarea w-full p-4 border bg-black border-purple-400 rounded-md h-32 resize-none focus:ring-2 focus:ring-purple-400"
            variants={fadeIn("up", 0.6)} // Shuffle effect
            initial="hidden"
            animate={inView ? "show" : "hidden"}
          ></motion.textarea>
          ...
          <motion.button
            ...
            initial="hidden"
            animate={inView ? "show" : "hidden"} // Control animation based on visibility
            className="bg-purple-600 text-white rounded-full py-3 px-8 max-w-[170px] mx-auto hover:shadow-lg hover:bg-purple-700 transition-all duration-300 flex items-center justify-center relative group"
          >
