...
// Service data array with type annotations
    ...
    >
      {serviceData.map((item, index) => (
        <SwiperSlide key={index}>
          <div className="bg-[#313131] text-[#FFF0D1] h-max rounded-lg px-6 py-8 flex flex-col gap-x-6 sm:gap-x-0 group cursor-pointer hover:bg-[#212121] transition-all duration-300">
            {/* Animated purple line */}
            <div className="w-full h-[4px] bg-[#FFF0D1] mb-4 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 ease-in-out"></div>
            ...
            <div className="mb-8">
              ...
              <p className="max-w-[350px] text-[10px] leading-normal text-black hover:text-[#FFF0D1] transition-all duration-300">{item.description}</p>
            </div>
            {/* arrow */}
            <div className="text-3xl">
              <RxArrowTopRight className="group-hover:rotate-45 text-[#FFF0D1] transition-all duration-300" />
            </div>
          </div>
        </SwiperSlide>
      ))}
